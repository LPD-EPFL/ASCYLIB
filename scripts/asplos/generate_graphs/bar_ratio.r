...
theme_set(theme_bw())
 ...
 mvh=max(high$ratio)*1.4
 mvl=max(low$ratio)*1.4
p1=ggplot(data=high, aes(x=machine, y=ratio, fill=structure)) +xlab("")+ylab("Ratio v. sequential") +geom_bar(stat="identity", colour="black",position=position_dodge())+ggtitle("High contention")+geom_text(angle = 90, aes(label = paste(sprintf("%.1f", scalability), "", sep=""), y =ratio+mvh/9), size = 3.7, position = position_dodge(width=0.9)) +scale_fill_brewer(palette="OrRd") + scale_x_discrete(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0),limits= c(0,mvh)) + theme(legend.key.size = unit(0.45, "cm"))+theme(legend.text = element_text(size = 13))+theme(legend.title = element_text(size = 13))+theme(axis.text.y = element_text(size = )) + theme(axis.text.x = element_text(size = 12)) + opts( panel.grid.major = theme_blank(),panel.grid.minor = theme_blank())  + theme(panel.border= element_rect(colour="black"))
p2=ggplot(data=low, aes(x=machine, y=ratio, fill=structure))+ggtitle("Low contention") +xlab("")+ylab("Ratio v. sequential") +geom_bar(stat="identity", colour="black",position=position_dodge())+geom_text(angle = 90, aes(label = paste(sprintf("%.1f", scalability), "", sep=""), y =ratio+mvl/9), size = 3.7, position = position_dodge(width=0.9)) +scale_fill_brewer(palette="OrRd") + scale_x_discrete(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0),limits= c(0,mvl))+ theme(axis.text.y = element_text(size = )) + theme(axis.text.x = element_text(size = 12)) +  opts( panel.grid.major = theme_blank(),panel.grid.minor = theme_blank()) + theme(panel.border= element_rect(colour="black"))
