...
printf "#       %-62s%-62s\n" "$prog1" "$prog2";
echo "#cores  throughput  pow_all power   eng_all/op eng/op(uj) scalability throughput  pow_all power   eng_all/op eng/op(uj) scalability";
...
pow_all=$(echo "$res" | awk '/CONSUMED Total power/ { print $6 };');
eop=$(echo "$res" | awk '/#Energy per Operation/ { print $8 }' | sed 's/)//g');
eop_all=$(echo "$res" | awk '/#Energy per Operation/ { print $5 }' | sed 's/)//g');
printf "%-12d" $thr1a;
printf "%-8.3f%-8.3f%-11.6f%-11.6f" $pow_all $pow $eop_all $eop;
printf "%-12d" 1;
