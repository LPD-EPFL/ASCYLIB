...
p2 = ggplot(data=mydata2, aes(y=throughput, x=cores, shape=structure, group=structure, colour=structure)) + geom_line(size=0.5) + geom_point(dat=mydata2[(mydata2$cores+c(mydata2$structure))%%10==0,],size=3, fill="white") +  facet_grid(.~machine,scales="free_x")+xlab("# threads") + ylab("Throughput (Mops/s)") +scale_fill_brewer(palette="OrRd") + scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))+scale_shape_manual(values= c(0,1,2,3,4,5,15,16,17,18)) 
