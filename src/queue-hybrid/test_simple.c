...
#define DS_ADD(rpc,server,key,val)       server_push(rpc,server,key,val)
#define DS_REMOVE(rpc,server)        server_pop(rpc,server)
...
typedef struct queue_rpc
{
  size_t type;
  skey_t key;
  sval_t val;
  uint8_t padding[CACHE_LINE_SIZE - sizeof(size_t) - sizeof(skey_t) - sizeof(sval_t) - sizeof(uint64_t)];
  union 
  {
...
int 
server_push(queue_rpc_t* rpc, int server, skey_t key, sval_t val)
{
  rpc->type = PUSH;
  rpc->key = key;
  rpc->val = val;
  ssmp_send(server, (ssmp_msg_t*) rpc);
  ssmp_recv_from(server, (ssmp_msg_t*) rpc);
  return rpc->val;
}

sval_t
server_pop(queue_rpc_t* rpc, int server)
{
  rpc->type = POP;
  ssmp_send(server, (ssmp_msg_t*) rpc);
  ssmp_recv_from(server, (ssmp_msg_t*) rpc);
...
test_server(void* thread)
{
  ...
  queue_rpc_t* rpc = (queue_rpc_t*) memalign(CACHE_LINE_SIZE, sizeof(queue_rpc_t));
  assert(rpc != NULL);
  ssmp_color_buf_t* cbuf = (ssmp_color_buf_t*) memalign(CACHE_LINE_SIZE, sizeof(ssmp_color_buf_t));
  assert(cbuf != NULL);
  ssmp_color_buf_init(cbuf, clients);

  while (1)
    {
      ssmp_recv_color_start(cbuf, (ssmp_msg_t*) rpc);
      /* ssmp_msg_t* msg = (ssmp_msg_t*) rpc; */
      if (rpc->type == PUSH)
	{
	  /* printf("++ from %d / key: %zu\n", msg->sender, rpc->key); */
	  int res = queue_add_seq(set, rpc->key, rpc->val);
	  rpc->val = res;
	}
      else
	{
	  sval_t res = queue_remove_seq(set);
	  rpc->val = res;
	}
      ssmp_send(rpc->sender, (ssmp_msg_t*) rpc);
    }
  ...
  for(i = 0; i < num_elems_thread; i++) 
    {
      ...
      if(DS_ADD(rpc, server, key, key) == false)
	{
	  i--;
	}
    }
...
main(int argc, char **argv) 
{
  ...
  thread_data_t* tds = (thread_data_t*) malloc((num_threads + 1) * sizeof(thread_data_t));

  tds[num_threads].id = num_threads;
  tds[num_threads].set = set;
  rc = pthread_create(&threads[num_threads], &attr, test_server, tds + num_threads);
  if (rc)
    {
