...
#define PQUEUE_MAX_THREADS 64

typedef ALIGNED(CACHE_LINE_SIZE) struct pqueue
{
  size_t size;
  uint8_t padding1[CACHE_LINE_SIZE - sizeof(size_t)];
  queue_low_t queues[PQUEUE_MAX_THREADS];
} pqueue_t;
...
pqueue_t* pqueue_new();
void pqueue_delete(pqueue_t* qu);
int pqueue_size(pqueue_t* cqu);
