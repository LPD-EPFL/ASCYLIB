...
result_t attempt_get(bst_key_t k, node_t* node, int direction, uint64_t node_v);
void bst_print(node_t* node);

uint64_t bst_size(node_t* node);

// checked
static inline node_t* CHILD(node_t* parent, int direction) {
	return (direction > 0 ? parent->right : parent->left);
}
