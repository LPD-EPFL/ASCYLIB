...
typedef mstack_node_t* mstack_req_t;

static inline uintptr_t
mstack_req_is_push(mstack_req_t r)
{
  return ((uintptr_t) r) & 0x1;
}

static inline mstack_req_t
mstack_req_set_push(mstack_req_t r)
{
  return (mstack_req_t) (((uintptr_t) r) | 0x1);
}

#define MSTACK_NUM_ELIM 8

typedef ALIGNED(CACHE_LINE_SIZE) struct mstack
{
  mstack_node_t* top;
  optik_t lock;
  uint8_t padding2[CACHE_LINE_SIZE - sizeof(optik_t) - sizeof(mstack_node_t*)];
  mstack_req_t elim[MSTACK_NUM_ELIM * MSTACK_NUM_ELIM];
} mstack_t;
