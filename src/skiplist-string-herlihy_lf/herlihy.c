...
int				
fraser_search(sl_intset_t *set, strkey_t key, sl_node_t **left_list, sl_node_t **right_list)
{
  int i;
 ...
 retry:
  ...
  for (i = levelmax - 1; i >= 0; i--)
      ...
      for (right = left_next; ; right = right_next)
	{
	  ...
	  // if (right->key >= key)
    if (strkey_compare(right->key, key) >= 0)  
	    {
	      break;
	    }
	  left = right; 
  ...
  return (strkey_compare(right->key, key) == 0);
}

int				
fraser_search_no_cleanup(sl_intset_t *set, strkey_t key, sl_node_t **left_list, sl_node_t **right_list)
{
  PARSE_TRY();
...
int				
fraser_search_no_cleanup_succs(sl_intset_t *set, strkey_t key, sl_node_t **right_list)
{
  PARSE_TRY();
...
static sl_node_t* 
fraser_left_search(sl_intset_t *set, strkey_t key)
{
  PARSE_TRY();
  sl_node_t* left = NULL;
  sl_node_t* left_prev;
  ...
  for (lvl = levelmax - 1; lvl >= 0; lvl--)
    {
      ...
      while(strkey_compare(left->key, key) < 0 || IS_MARKED(left->next[lvl]))
      	{
      ...
      // if ((left->key == key))
        if (strkey_compare(left->key, key) == 0)
	{
	  break;
	}
    }
...
strval_t
fraser_find(sl_intset_t *set, strkey_t key)
{
  strval_t result = {""};
  PARSE_START_TS(0);
...
strval_t
fraser_remove(sl_intset_t *set, strkey_t key)
{
  UPDATE_TRY();
...
int
fraser_insert(sl_intset_t *set, strkey_t key, strval_t val) 
{
