...
#define ELIM_SPOTS 32

typedef ALIGNED(CACHE_LINE_SIZE) struct mstack
{
  mstack_node_t* top;
  optik_t lock;
  uint8_t padding2[CACHE_LINE_SIZE - sizeof(mstack_node_t*)];
  volatile mstack_node_t* pushe[ELIM_SPOTS];
  volatile mstack_node_t* pope[ELIM_SPOTS];
} mstack_t;
