...
chm_put(chm_t* set, skey_t key, sval_t val)
{
  ...
  while (1)
  {
    ...
    volatile chm_seg_t* seg = set->segments[seg_num];
    optik_t* seg_lock = &seg->lock;
    COMPILER_NO_REORDER(optik_t version = *seg_lock;);
    ...
    if (unlikely(!optik_trylock_version(seg_lock, version)))
      {
	continue;
      }
...
chm_rem(chm_t* set, skey_t key)
{
 ...
 restart:
  {
    volatile int seg_num = key & set->hash;
    volatile chm_seg_t* seg = set->segments[seg_num];
    optik_t* seg_lock = &seg->lock;
    COMPILER_NO_REORDER(optik_t version = *seg_lock;);
