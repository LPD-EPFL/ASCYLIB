...
ht_new() 
{
  ...
  size_t bs = (maxhtlength + 1) * sizeof(map_t);
  ...
  for (i = 0; i < maxhtlength; i++) 
    {
      map_init_l(&set->buckets[i], maxbulength);
    }
  return set;
}
...
ht_contains(ht_intset_t *set, skey_t key) 
{
  ...
  return map_contains_l(&set->buckets[addr], key);
}
...
{
  int addr = key & set->hash;
  return map_add_l(&set->buckets[addr], key, val);
}
...
{
  int addr = key & set->hash;
  return map_remove_l(&set->buckets[addr], key);
}
