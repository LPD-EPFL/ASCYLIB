...
extern fallback_flag_t fallback;

typedef struct thread_data
{
  uint32_t id;
  fallback_flag_t *flag;
  DS_TYPE* set;
} thread_data_t;
  ...
  while (stop == 0) 
    {
      ...
      if (qcount % QUIESCENCE_THRESHOLD == 0){
...
main(int argc, char **argv) 
{
  ...
  sleeper_thread_data_t* slthds = (sleeper_thread_data_t *) malloc(num_cores * sizeof(sleeper_thread_data_t));
